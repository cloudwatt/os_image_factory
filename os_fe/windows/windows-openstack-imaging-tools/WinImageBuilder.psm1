...
function Create-BCDBootConfig {
    ...
    if ($diskLayout -eq "BIOS") {
        ...
        if ($LASTEXITCODE) { Write-Warning "BCDEdit failed: bootmgr device locate. Exit code: $LASTEXITCODE" }

        & $bcdeditPath /store ${systemDrive}\boot\BCD /set `{default`} device locate
        if ($LASTEXITCODE) { Write-Warning "BCDEdit failed: default device locate. Exit code: $LASTEXITCODE" }

        & $bcdeditPath /store ${systemDrive}\boot\BCD /set `{default`} osdevice locate
        if ($LASTEXITCODE) { Write-Warning "BCDEdit failed: default osdevice locate. Exit code: $LASTEXITCODE" }
    }
}
...
function Add-DriversToImage {
    ...
    if ($LASTEXITCODE) {
        throw "Dism failed to add drivers from: $driversPath. Last exit code: $LASTEXITCODE"
    }
}
